---
name: workflow
description: Use whenever starting a new software project or a substantial new feature — before writing any code. Replaces vibe coding with a Spec → Break → Plan → Execute loop (spec-driven development), the same pattern formalized in 2026 by GitHub Spec Kit, OpenSpec, AWS Kiro and Codex Skills. Trigger on "novo projeto", "vamos construir/criar um app/sistema/feature", "bora começar um projeto do zero", or any request to build something non-trivial with AI.
metadata:
...
Validado contra a prática de mercado de 2026 (GitHub Spec Kit, OpenSpec, AWS Kiro, Codex Skills) — não é uma convenção inventada, é o padrão que a indústria convergiu para resolver exatamente estes 4 problemas.
...
Na fase `/execute`, não use um único agente genérico para tudo. Identifique as camadas que a issue toca (ex.: modelo/dados, rota/endpoint, componente de UI, hook, integração externa, testes) e use ou crie em `.Codex/agents/` um subagente por camada, no padrão `<camada>-writer.md`, cada um com escopo estreito e as skills relevantes daquela camada. Isso mantém cada agente com contexto pequeno e focado, e reforça o isolamento por comportamento.
...
- `.Codex/agents/*-writer.md` — subagentes por camada, criados sob demanda.
